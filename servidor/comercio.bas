...
on error goto errorh
    ...
    if objdata(obji).real = 1 then
        if npclist(npcindex).name <> "sr" then
            call senddata(sendtarget.topcarea, userindex, userlist(userindex).pos.map, "||" & vbwhite & "�" & "lo siento, la ropa faccionaria solo es para muestra, no tengo autorizaci�n para venderla. dir�jete al sastre de tu ej�rcito." & "�" & str(npclist(npcindex).char.charindex))
            exit function
        end if
    end if
...
on error goto errhandler
    ...
    if objdata(userlist(userindex).invent.object(item).objindex).real = 1 then
        if npclist(npcindex).name <> "sr" then
            call senddata(sendtarget.toindex, userindex, 0, "||las armaduras faccionarias s�lo las puedes vender a sus respectivos sastres" & fonttype_warning)
